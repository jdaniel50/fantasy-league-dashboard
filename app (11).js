...
async function renderDraftTab() {
  ...
  // Calculate pick ratings
  const pickRatings = calculateDraftPickRatings(draftPicks, matchupsByWeek, rosters, selectedSeason);
...
function calculateDraftPickRatings(draftPicks, matchupsByWeek, rosters, season) {
  if (!matchupsByWeek || !playersData) return [];
  
  const positions = ['QB', 'RB', 'WR', 'TE'];
  const pickRatings = [];
  
  // Calculate total points for each player in starting lineups
  const playerSeasonPoints = new Map();
...
function renderDraftBoard(draftPicks, pickRatings, rosters, season) {
  ...
  const teamOrder = sortedPicks.slice(0, numTeams).map(p => p.roster_id);
  ...
  let html = '<div class="draft-board-title">Draft Board</div>';
  ...
  for (let round = 1; round <= numRounds; round++) {
    ...
    // Snake draft logic
    const roundTeamOrder = round % 2 === 1 ? teamOrder : [...teamOrder].reverse();
        ...
        if (player) {
          html += `
            <div class="draft-pick-card">
              <div class="draft-pick-name">${escapeHtml(player.full_name || 'Unknown')}</div>
              <div class="draft-pick-position">${player.position || '??'} - Pick ${pick.pick_no}</div>
              ${rating ? `<div class="draft-pick-rating ${rating.ratingTier}">${rating.ratingLabel}</div>` : ''}
            </div>
          `;
        } else {
