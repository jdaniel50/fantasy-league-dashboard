  ...
  trades.forEach(trade => {
    ...
    if (trade.adds && trade.drops) {
      Object.keys(trade.adds).forEach(playerId => {
        ...
        const oldRosterId = Object.keys(trade.roster_ids || {}).find(rid => {
          return trade.roster_ids[rid] === 1; // Find the roster that gave up the player
        });
...
function renderDraftPicksCards(pickRatings, evalWeek) {
  ...
  let html = '<div class="awards-grid" style="grid-template-columns: repeat(4, 1fr);">';
