...
// ============================================================================
// GLOBAL ADMIN - TEAM NAME & COLOR MANAGEMENT
// ============================================================================
...
function renderGlobalAdminPanel() {
  ...
  if (!activeLeagueId) {
    container.innerHTML = '<div class="muted-text">Please select a league first.</div>';
    return;
  }
  ...
  // Collect all unique user_ids across all seasons
  const allUserIds = new Set();
  const userInfo = new Map(); // user_id -> { displayName, seasons }
  ...
  histData.seasons.forEach(seasonInfo => {
    ...
    if (rosters) {
      rosters.forEach(roster => {
        if (roster.owner_id) {
          allUserIds.add(roster.owner_id);
          
          if (!userInfo.has(roster.owner_id)) {
            const user = currentUsers.find(u => u.user_id === roster.owner_id);
            userInfo.set(roster.owner_id, {
              displayName: user ? (user.display_name || user.username || roster.owner_id) : roster.owner_id,
              seasons: []
            });
          }
          
          userInfo.get(roster.owner_id).seasons.push(season);
        }
      });
    }
  });
  
  // Render each user's settings
  let html = '';
  
  Array.from(allUserIds).sort((a, b) => {
    const nameA = userInfo.get(a).displayName.toLowerCase();
    const nameB = userInfo.get(b).displayName.toLowerCase();
    ...
    html += `
      <div class="team-settings-row">
        <div>
          <div class="team-settings-label">Original Name</div>
          <div style="color: var(--text-primary); font-size: 14px; margin-top: 4px;">${escapeHtml(info.displayName)}</div>
          <div style="color: var(--text-muted); font-size: 11px; margin-top: 2px;">${info.seasons.length} season(s)</div>
        </div>
        <input 
          type="text" 
          class="team-settings-input" 
          placeholder="Custom name (leave blank for original)"
...
function saveTeamSettings(userId) {
  ...
  // Refresh all tabs to show changes
  renderHistoryTab();
  renderAwardsTab();
  renderDraftTab();
  renderMatchupTab();
...
// Event listeners for global admin
document.addEventListener('DOMContentLoaded', () => {
  const globalAdminBtn = document.getElementById('globalAdminBtn');
  ...
  if (globalAdminBtn) {
    globalAdminBtn.addEventListener('click', openGlobalAdmin);
  }
  
  if (exportBtn) {
    exportBtn.addEventListener('click', exportGlobalTeamSettings);
  }
  
  if (importBtn) {
    importBtn.addEventListener('click', importGlobalTeamSettings);
  }
