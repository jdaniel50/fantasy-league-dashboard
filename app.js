...
function populateSeasonSelectors(seasons) {
  ...
  // Awards season select
  awardsSeasonSelect.innerHTML = '<option value="">Select a season...</option>';
  seasons.forEach(season => {
    const option = document.createElement('option');
    option.value = season.season;
    option.textContent = `${season.season} Season`;
  ...
  // Auto-select current season for awards
  if (seasons.length > 0) {
...
function calculateHistoricalStats(histData, selectedSeason, teamSettings) {
  ...
  seasonsToProcess.forEach(seasonInfo => {
    ...
    // Count playoff finishes based on final standings
    if (rosters) {
      rosters.forEach(roster => {
        ...
        if (stats.has(userId)) {
          ...
          // Check metadata for playoff results
          if (roster.metadata) {
            if (roster.metadata.p1 === '1') userStats.first_place++;
            else if (roster.metadata.p2 === '1') userStats.second_place++;
            else if (roster.metadata.p3 === '1') userStats.third_place++;
          }
          
          // Fallback: use settings.rank if available
          if (seasonInfo.settings && seasonInfo.settings.playoff_week_start) {
            // Count playoff wins from matchups
            const playoffStart = seasonInfo.settings.playoff_week_start;
            const seasonMatchups = histData.allMatchups.get(season);
            
            if (seasonMatchups) {
              let playoffWins = 0;
              seasonMatchups.forEach((weekMatchups, week) => {
                if (week >= playoffStart) {
                  weekMatchups.forEach(matchup => {
                    if (matchup.roster_id === roster.roster_id && matchup.matchup_id) {
                      const opponent = weekMatchups.find(m => 
                        m.matchup_id === matchup.matchup_id && m.roster_id !== matchup.roster_id
                      );
              ...
              userStats.playoff_wins += playoffWins;
            }
          }
        }
      });
...
async function calculateSeasonAwards(histData, season) {
  ...
  // 5. BEST TRADE (Placeholder - requires detailed player tracking)
  if (trades && trades.length > 0) {
    awards.push({
      ...
      teamColor: '#f5c451',
      description: 'Trade analysis requires detailed player performance tracking across the season. Feature in development!'
    });
  }
  
  // 6. BEST DRAFT PICKS (Placeholder - requires detailed player tracking)
  if (draftPicks && draftPicks.length > 0) {
    awards.push({
      ...
      team: 'Analysis Coming Soon',
      teamColor: '#f5c451',
      description: 'Draft value analysis requires player rankings and season statistics. Feature in development!'
    });
  }
  
  // 7. BEST WAIVER PICKUP (Placeholder - requires detailed player tracking)
  if (waivers && waivers.length > 0) {
    awards.push({
      ...
      team: 'Analysis Coming Soon',
      teamColor: '#f5c451',
      description: 'Waiver analysis requires player performance tracking post-acquisition. Feature in development!'
    });
  }
