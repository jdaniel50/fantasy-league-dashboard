...
const powerSeasonSelect = document.getElementById('powerSeasonSelect');
const powerWeekSelect = document.getElementById('powerWeekSelect');
...
function initControls() {
  ...
  if (standingsSeasonSelect) {
    standingsSeasonSelect.addEventListener('change', () => {
      renderStandingsTab();
    });
  }
  
  // Power rankings season select
  if (powerSeasonSelect) {
    powerSeasonSelect.addEventListener('change', () => {
      populatePowerWeekSelect();
      renderPowerTab();
    });
  }
  
  // Power rankings week select
  if (powerWeekSelect) {
    powerWeekSelect.addEventListener('change', () => {
      renderPowerTab();
    });
  }
...
async function loadHistoricalData(leagueId) {
  ...
  try {
    ...
    while (currentId && !visited.has(currentId)) {
      ...
      const usersRes = await fetch(`https://api.sleeper.app/v1/league/${currentId}/users`);
      if (usersRes.ok) {
        const users = await usersRes.json();
        histData.allUsers.set(league.season, users);
      }
    ...
    // Build comprehensive user map (user_id -> user info from any season they appeared)
    const comprehensiveUsers = new Map();
    histData.allUsers.forEach((seasonUsers, season) => {
      seasonUsers.forEach(user => {
        if (!comprehensiveUsers.has(user.user_id)) {
          comprehensiveUsers.set(user.user_id, user);
        }
      });
    });
    
    // Update currentUsers with comprehensive list from all seasons
    currentUsers = Array.from(comprehensiveUsers.values());
...
function populateSeasonSelectors(seasons) {
  ...
  // Standings season select
  if (standingsSeasonSelect) {
    standingsSeasonSelect.innerHTML = '<option value="">Select a season...</option>';
    seasons.forEach(season => {
      const option = document.createElement('option');
      option.value = season.season;
      option.textContent = `${season.season} Season`;
      standingsSeasonSelect.appendChild(option);
    });
    // Auto-select most recent season
    if (seasons.length > 0) {
      standingsSeasonSelect.value = seasons[0].season;
    }
  }
  
  // Power Rankings season select
  if (powerSeasonSelect) {
    powerSeasonSelect.innerHTML = '<option value="">Select a season...</option>';
    seasons.forEach(season => {
      const option = document.createElement('option');
      option.value = season.season;
      option.textContent = `${season.season} Season`;
      powerSeasonSelect.appendChild(option);
    });
    // Auto-select most recent season
    if (seasons.length > 0) {
      powerSeasonSelect.value = seasons[0].season;
    }
  }
...
function populatePowerWeekSelect() {
  if (!powerWeekSelect || !activeLeagueId) return;
  
  const selectedSeason = powerSeasonSelect?.value;
  
  if (!selectedSeason) {
    powerWeekSelect.innerHTML = '<option value="">Select a week...</option>';
    return;
  }
  ...
  const weeks = Array.from(matchupsByWeek.keys())
    .filter(w => w < playoffStart) // Only show regular season weeks
    .sort((a, b) => a - b);
  ...
  weeks.forEach(week => {
    ...
    powerWeekSelect.appendChild(option);
  });
  
  // Auto-select most recent week
  if (weeks.length > 0) {
    powerWeekSelect.value = weeks[weeks.length - 1];
  }
}
  ...
  seasonsToProcess.forEach(seasonInfo => {
    ...
    if (seasonInfo.settings && seasonInfo.settings.playoff_week_start && matchupsByWeek) {
      ...
      if (playoffWeeks.length > 0) {
        ...
        // Determine which teams made playoffs based on regular season standings
        const regularSeasonStandings = new Map(); // roster_id -> { wins, points }
        
        matchupsByWeek.forEach((weekMatchups, week) => {
          if (week < playoffStart) { // Only regular season
            weekMatchups.forEach(matchup => {
              if (!matchup.matchup_id) return;
              
              if (!regularSeasonStandings.has(matchup.roster_id)) {
                regularSeasonStandings.set(matchup.roster_id, { wins: 0, points: 0 });
              }
              
              const standing = regularSeasonStandings.get(matchup.roster_id);
              standing.points += (matchup.points || 0);
        ...
        // Get top X teams that made playoffs (sorted by wins, then points)
        const playoffTeamRanking = Array.from(regularSeasonStandings.entries())
          .sort((a, b) => {
        ...
        const playoffRosterIds = new Set(playoffTeamRanking);
        
        // Find championship matchup - try multiple strategies
        let champMatchups = null;
        
        // Strategy 1: matchup_id 1
        let candidates = championshipWeek.matchups.filter(m => m.matchup_id === 1);
        if (candidates.length === 2 && 
            playoffRosterIds.has(candidates[0].roster_id) && 
            playoffRosterIds.has(candidates[1].roster_id)) {
          champMatchups = candidates;
        }
        
        // Strategy 2: Find matchup between top 2 seeded teams
        if (!champMatchups && playoffTeamRanking.length >= 2) {
          const top1 = playoffTeamRanking[0];
          const top2 = playoffTeamRanking[1];
          
          for (let matchupId of [1, 2, 3, 4]) {
            candidates = championshipWeek.matchups.filter(m => m.matchup_id === matchupId);
            if (candidates.length === 2) {
              const rosterIds = new Set(candidates.map(m => m.roster_id));
              if (rosterIds.has(top1) && rosterIds.has(top2)) {
                champMatchups = candidates;
                break;
              }
            }
        ...
        // Strategy 3: Highest combined score among playoff teams
        if (!champMatchups) {
          const matchupsByIdMap = new Map();
          championshipWeek.matchups.forEach(m => {
            if (!m.matchup_id || !playoffRosterIds.has(m.roster_id)) return;
            if (!matchupsByIdMap.has(m.matchup_id)) {
              matchupsByIdMap.set(m.matchup_id, []);
            }
            matchupsByIdMap.get(m.matchup_id).push(m);
          });
          
          let highestScore = 0;
          matchupsByIdMap.forEach((teams) => {
            if (teams.length === 2) {
              const totalScore = (teams[0].points || 0) + (teams[1].points || 0);
        ...
        // Award championship and 2nd place
        if (champMatchups && champMatchups.length === 2) {
          ...
          if (winnerRoster && winnerRoster.owner_id && stats.has(winnerRoster.owner_id)) {
          ...
          if (loserRoster && loserRoster.owner_id && stats.has(loserRoster.owner_id)) {
        ...
        // Find 3rd place game - similar multi-strategy approach
        let thirdPlaceMatchups = null;
        
        // Strategy 1: matchup_id 2
        candidates = championshipWeek.matchups.filter(m => m.matchup_id === 2);
        if (candidates.length === 2 && 
            playoffRosterIds.has(candidates[0].roster_id) && 
            playoffRosterIds.has(candidates[1].roster_id)) {
          thirdPlaceMatchups = candidates;
        }
        
        // Strategy 2: Second highest combined score among playoff teams
        if (!thirdPlaceMatchups) {
          const matchupsByIdMap = new Map();
          championshipWeek.matchups.forEach(m => {
            if (!m.matchup_id || !playoffRosterIds.has(m.roster_id)) return;
            // Skip championship matchup
            if (champMatchups && (m.roster_id === champMatchups[0].roster_id || m.roster_id === champMatchups[1].roster_id)) return;
          ...
          matchupsByIdMap.forEach((teams) => {
            if (teams.length === 2) {
              const totalScore = (teams[0].points || 0) + (teams[1].points || 0);
              if (totalScore > secondHighest) {
                secondHighest = totalScore;
                thirdPlaceMatchups = teams;
              }
            }
          });
        }
        ...
        // Award 3rd place
        if (thirdPlaceMatchups && thirdPlaceMatchups.length === 2) {
        ...
        // Count playoff wins - ONLY competitive wins in main bracket
        // For each playoff week, only count wins for teams that are:
        // 1. In the main playoff bracket (playoffRosterIds)
        // 2. Playing for advancement (not consolation after losing)
        
        // Track which teams have lost (playing consolation)
        ...
        playoffWeeks.forEach(({ week, matchups }) => {
          matchups.forEach(matchup => {
            ...
            // Only consider teams in main playoff bracket
            if (!playoffRosterIds.has(matchup.roster_id)) return;
            
            // Skip if this team is already eliminated (playing consolation)
            if (eliminatedTeams.has(matchup.roster_id)) return;
            ...
            if (opponent && stats.has(roster.owner_id)) {
              const won = (matchup.points || 0) > (opponent.points || 0);
              const lost = (matchup.points || 0) < (opponent.points || 0);
              
              // Count this win (team is still competing for advancement)
              if (won) {
                stats.get(roster.owner_id).playoff_wins++;
              }
              
              // Mark loser as eliminated ONLY if not in championship week
              // (championship week losers get 2nd/3rd place, not eliminated)
              if (lost && week !== championshipWeek.week) {
                eliminatedTeams.add(matchup.roster_id);
              }
            }
          });
        });
...
async function renderAwardsSeasonTab() {
  ...
  const seasonSelect = document.getElementById('awardsSeasonSeasonSelect');
  ...
  if (!selectedSeason) {
    content.innerHTML = '<div class="muted-text">Select a season to view awards.</div>';
    return;
  }
  ...
  content.innerHTML = '<div class="muted-text">Calculating season awards...</div>';
  
  // Calculate awards for this season
  ...
  // Best of the Year
  html += '<div class="awards-section-title">🌟 Best of the Year</div>';
  html += '<div class="awards-grid-full">';
  html += renderAwardCard('Most Points (Season)', awards.bestOfYear.mostPointsSeason);
  html += renderAwardCard('Most Points (Week)', awards.bestOfYear.mostPointsWeek);
  html += renderAwardCard('Biggest Blowout', awards.bestOfYear.biggestBlowout);
  html += renderAwardCard('Luckiest Team', awards.bestOfYear.luckiestTeam);
  html += renderAwardCard('Highest Scoring QB', awards.bestOfYear.topQB);
  html += renderAwardCard('Highest Scoring RB', awards.bestOfYear.topRB);
  html += renderAwardCard('Highest Scoring WR', awards.bestOfYear.topWR);
  html += renderAwardCard('Highest Scoring TE', awards.bestOfYear.topTE);
  html += '</div>';
  
  // Worst of the Year
  html += '<div class="awards-section-title">📉 Worst of the Year</div>';
  html += '<div class="awards-grid-full">';
  html += renderAwardCard('Least Points (Season)', awards.worstOfYear.leastPointsSeason);
  html += renderAwardCard('Least Points (Week)', awards.worstOfYear.leastPointsWeek);
  html += renderAwardCard('Biggest Loss', awards.worstOfYear.biggestLoss);
  html += '</div>';
...
// ============================================================================
// SEASON AWARDS CALCULATION
// ============================================================================

async function calculateSeasonAwards(histData, selectedSeason) {
  const teamSettings = getTeamSettings(activeLeagueId);
  ...
  if (!matchupsByWeek || !rosters || !seasonInfo) {
    return { bestOfYear: {}, worstOfYear: {}, tradesWaivers: {} };
  }
  ...
  // Initialize tracking
  let mostPointsSeason = { points: 0, team: null, roster: null };
  let mostPointsWeek = { points: 0, team: null, week: null, roster: null };
  let leastPointsSeason = { points: Infinity, team: null, roster: null };
  let leastPointsWeek = { points: Infinity, team: null, week: null, roster: null };
  let biggestBlowout = { margin: 0, winner: null, loser: null, week: null, winnerScore: 0, loserScore: 0 };
  let biggestLoss = { margin: 0, winner: null, loser: null, week: null, winnerScore: 0, loserScore: 0 };
  
  // Player scoring tracking
  const playerSeasonScoring = []; // { playerId, position, points, week, rosterId }
  
  // Waiver tracking
  const bestWaivers = { QB: null, RB: null, WR: null, TE: null };
  
  // Track season totals and all-play records
  const seasonTotals = new Map(); // roster_id -> { total: points, allPlayWins: 0, allPlayLosses: 0, wins: 0 }
  rosters.forEach(r => {
    seasonTotals.set(r.roster_id, { total: 0, allPlayWins: 0, allPlayLosses: 0, wins: 0 });
  });
  ...
  matchupsByWeek.forEach((weekMatchups, week) => {
    ...
    weekMatchups.forEach(matchup => {
      ...
      const totals = seasonTotals.get(matchup.roster_id);
      ...
      if (points > mostPointsWeek.points) {
        mostPointsWeek = { points, team: roster, week, roster };
      }
      if (points < leastPointsWeek.points && points > 0) {
        leastPointsWeek = { points, team: roster, week, roster };
      }
      ...
      // Calculate all-play record (regular season only)
      if (isRegularSeason) {
        weekMatchups.forEach(opp => {
          if (opp.roster_id !== matchup.roster_id) {
            if (points > (opp.points || 0)) {
              totals.allPlayWins++;
            } else if (points < (opp.points || 0)) {
              totals.allPlayLosses++;
            }
          }
        });
      }
      ...
      if (matchup.matchup_id) {
        ...
        if (opponent) {
          const margin = Math.abs(points - (opponent.points || 0));
          const isWinner = points > (opponent.points || 0);
          
          // Track wins (regular season only)
          if (isRegularSeason && isWinner) {
            totals.wins++;
          }
  ...
  // Check season totals
  seasonTotals.forEach((totals, rosterId) => {
    const roster = rosterByRosterId.get(rosterId);
    if (roster) {
      if (totals.total > mostPointsSeason.points) {
        mostPointsSeason = { points: totals.total, team: roster, roster };
      }
      if (totals.total < leastPointsSeason.points) {
        leastPointsSeason = { points: totals.total, team: roster, roster };
      }
    }
  });
  
  // Find luckiest team (best record, worst all-play)
  let luckiestTeam = null;
  let maxLuckScore = -Infinity;
  
  seasonTotals.forEach((totals, rosterId) => {
    const roster = rosterByRosterId.get(rosterId);
    if (roster && totals.wins > 0) {
      const allPlayWinPct = totals.allPlayWins / (totals.allPlayWins + totals.allPlayLosses);
      const recordWinPct = totals.wins / (totals.wins + (rosters.length - 1 - totals.wins)); // Approximate
      const luckScore = recordWinPct - allPlayWinPct; // Positive = lucky
      
      if (luckScore > maxLuckScore) {
        maxLuckScore = luckScore;
        luckiestTeam = {
          roster,
          record: `${totals.wins}-${rosters.length - 1 - totals.wins}`,
          allPlayRecord: `${totals.allPlayWins}-${totals.allPlayLosses}`
        };
      }
    }
  });
  ...
  const playerSeasonTotals = new Map(); // playerId -> { points, rosterId }
  playerSeasonScoring.forEach(entry => {
    if (!playerSeasonTotals.has(entry.playerId)) {
      playerSeasonTotals.set(entry.playerId, { points: 0, rosterId: entry.rosterId });
    }
    playerSeasonTotals.get(entry.playerId).points += entry.points;
  });
  
  // Find top player by position
  const topPlayers = { QB: null, RB: null, WR: null, TE: null };
  
  playerSeasonTotals.forEach((data, playerId) => {
    const player = playersData[playerId];
    if (!player || !['QB', 'RB', 'WR', 'TE'].includes(player.position)) return;
    
    const roster = rosterByRosterId.get(data.rosterId);
    if (!roster) return;
  ...
  waivers.forEach(transaction => {
    ...
    Object.keys(transaction.adds).forEach(playerId => {
      ...
      let totalPoints = 0;
      playerSeasonScoring
        .filter(p => p.playerId === playerId && p.week >= pickupWeek && p.rosterId === rosterId)
  ...
  return {
    bestOfYear: {
      mostPointsSeason: mostPointsSeason.team ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(mostPointsSeason.team)}</span> led the league with <span class="award-stat">${mostPointsSeason.points.toFixed(1)} points</span> this season.`
      } : null,
      ...
      mostPointsWeek: mostPointsWeek.team ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(mostPointsWeek.team)}</span> exploded for <span class="award-stat">${mostPointsWeek.points.toFixed(1)} points</span> in Week ${mostPointsWeek.week}, the highest single-week score this season.`
      } : null,
      ...
      biggestBlowout: biggestBlowout.winner ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(biggestBlowout.winner)}</span> crushed <span class="award-stat">${getTeamName(biggestBlowout.loser)}</span> by <span class="award-stat">${biggestBlowout.margin.toFixed(1)} points</span> (${biggestBlowout.winnerScore.toFixed(1)}-${biggestBlowout.loserScore.toFixed(1)}) in Week ${biggestBlowout.week}.`
      } : null,
      
      luckiestTeam: luckiestTeam ? {
        winner: getTeamName(luckiestTeam.roster),
        teamName: getTeamName(luckiestTeam.roster),
        teamColor: getTeamColor(luckiestTeam.roster),
        blurb: `<span class="award-stat">${getTeamName(luckiestTeam.roster)}</span> had a record of <span class="award-stat">${luckiestTeam.record}</span> but an all-play record of only <span class="award-stat">${luckiestTeam.allPlayRecord}</span>.`
      } : null,
      ...
      topQB: topPlayers.QB ? {
        ...
        blurb: `<span class="award-stat">${topPlayers.QB.player.full_name}</span> (${getTeamName(topPlayers.QB.team)}) scored <span class="award-stat">${topPlayers.QB.points.toFixed(1)} points</span> this season.`
      } : null,
      ...
      topRB: topPlayers.RB ? {
        ...
        blurb: `<span class="award-stat">${topPlayers.RB.player.full_name}</span> (${getTeamName(topPlayers.RB.team)}) scored <span class="award-stat">${topPlayers.RB.points.toFixed(1)} points</span> this season.`
      } : null,
      ...
      topWR: topPlayers.WR ? {
        ...
        blurb: `<span class="award-stat">${topPlayers.WR.player.full_name}</span> (${getTeamName(topPlayers.WR.team)}) scored <span class="award-stat">${topPlayers.WR.points.toFixed(1)} points</span> this season.`
      } : null,
      ...
      topTE: topPlayers.TE ? {
        ...
        blurb: `<span class="award-stat">${topPlayers.TE.player.full_name}</span> (${getTeamName(topPlayers.TE.team)}) scored <span class="award-stat">${topPlayers.TE.points.toFixed(1)} points</span> this season.`
      } : null
    },
    ...
    worstOfYear: {
      leastPointsSeason: leastPointsSeason.team ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(leastPointsSeason.team)}</span> struggled with only <span class="award-stat">${leastPointsSeason.points.toFixed(1)} points</span> this season.`
      } : null,
      ...
      leastPointsWeek: leastPointsWeek.team ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(leastPointsWeek.team)}</span> put up a dismal <span class="award-stat">${leastPointsWeek.points.toFixed(1)} points</span> in Week ${leastPointsWeek.week}.`
      } : null,
      ...
      biggestLoss: biggestLoss.loser ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(biggestLoss.loser)}</span> got crushed by <span class="award-stat">${getTeamName(biggestLoss.winner)}</span> by <span class="award-stat">${biggestLoss.margin.toFixed(1)} points</span> (${biggestLoss.loserScore.toFixed(1)}-${biggestLoss.winnerScore.toFixed(1)}) in Week ${biggestLoss.week}.`
      } : null
    },
    ...
    tradesWaivers: {
      ...
      bestQBWaiver: bestWaivers.QB ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(bestWaivers.QB.team)}</span> picked up <span class="award-stat">${bestWaivers.QB.player.full_name}</span> in Week ${bestWaivers.QB.pickupWeek}, who scored <span class="award-stat">${bestWaivers.QB.points.toFixed(1)} points</span> in starting lineups.`
      } : null,
      ...
      bestRBWaiver: bestWaivers.RB ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(bestWaivers.RB.team)}</span> picked up <span class="award-stat">${bestWaivers.RB.player.full_name}</span> in Week ${bestWaivers.RB.pickupWeek}, who scored <span class="award-stat">${bestWaivers.RB.points.toFixed(1)} points</span> in starting lineups.`
      } : null,
      ...
      bestWRWaiver: bestWaivers.WR ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(bestWaivers.WR.team)}</span> picked up <span class="award-stat">${bestWaivers.WR.player.full_name}</span> in Week ${bestWaivers.WR.pickupWeek}, who scored <span class="award-stat">${bestWaivers.WR.points.toFixed(1)} points</span> in starting lineups.`
      } : null,
      ...
      bestTEWaiver: bestWaivers.TE ? {
        ...
        blurb: `<span class="award-stat">${getTeamName(bestWaivers.TE.team)}</span> picked up <span class="award-stat">${bestWaivers.TE.player.full_name}</span> in Week ${bestWaivers.TE.pickupWeek}, who scored <span class="award-stat">${bestWaivers.TE.points.toFixed(1)} points</span> in starting lineups.`
      } : null
    }
  };
}
...
// ============================================================================
// STANDINGS TAB RENDERING - WITH ALL-PLAY AND HISTORICAL SEASONS
// ============================================================================
...
async function renderStandingsTab() {
  ...
  const selectedSeason = standingsSeasonSelect?.value;
  
  if (!selectedSeason) {
    standingsContainer.innerHTML = '<div class="muted-text">Select a season to view standings.</div>';
    return;
  }
  ...
  if (!histData) {
    standingsContainer.innerHTML = '<div class="muted-text">Loading historical data...</div>';
    return;
  }
  ...
  if (!rosters || !matchupsByWeek || !seasonInfo) {
    standingsContainer.innerHTML = '<div class="muted-text">No data available for this season.</div>';
    return;
  }
  ...
  const allPlayRecords = new Map(); // roster_id -> { wins, losses }
  rosters.forEach(r => allPlayRecords.set(r.roster_id, { wins: 0, losses: 0 }));
  
  matchupsByWeek.forEach((weekMatchups, week) => {
    if (week >= playoffStart) return; // Only regular season
    
    weekMatchups.forEach(matchup => {
      const record = allPlayRecords.get(matchup.roster_id);
      if (!record) return;
  ...
  let html = `
    <div class="standings-section">
      <div class="standings-section-title">${selectedSeason ? `${selectedSeason} ` : ''}Standings</div>
  ...
  sortedRosters.forEach((roster, idx) => {
    ...
    const allPlay = allPlayRecords.get(roster.roster_id) || { wins: 0, losses: 0 };
    ...
    html += `
      <tr>
        ...
        <td>${allPlay.wins}-${allPlay.losses}</td>
...
// ============================================================================
// POWER RANKINGS TAB - COMPREHENSIVE WITH HISTORICAL TRACKING
// ============================================================================
...
async function renderPowerTab() {
  ...
  const selectedSeason = powerSeasonSelect?.value;
  const selectedWeek = powerWeekSelect?.value;
  
  if (!selectedSeason) {
    powerTableContainer.innerHTML = '<div class="muted-text">Select a season to view power rankings.</div>';
    return;
  }
  
  if (!selectedWeek) {
    powerTableContainer.innerHTML = '<div class="muted-text">Select a week to view power rankings.</div>';
    return;
  }
  ...
  if (!histData) {
    powerTableContainer.innerHTML = '<div class="muted-text">Loading historical data...</div>';
    return;
  }
  ...
  if (!rosters || !matchupsByWeek || !seasonInfo) {
    powerTableContainer.innerHTML = '<div class="muted-text">No data available for this season.</div>';
    return;
  }
  
  const playoffStart = seasonInfo.settings?.playoff_week_start || 15;
  const throughWeek = parseInt(selectedWeek);
  
  // Calculate power rankings for the selected week
  const currentRankings = calculatePowerRankingsForWeek(rosters, matchupsByWeek, throughWeek, playoffStart, teamSettings);
  
  // Calculate previous week's rankings for change tracking
  let previousRankings = [];
  if (throughWeek > 1) {
    previousRankings = calculatePowerRankingsForWeek(rosters, matchupsByWeek, throughWeek - 1, playoffStart, teamSettings);
  }
  ...
  currentRankings.forEach((team, idx) => {
    ...
    if (prevIdx >= 0) {
      team.change = prevIdx - idx; // Positive = moved up
  ...
  let html = `
    <div class="power-header">
      <div class="power-title">Power Rankings - ${selectedSeason} (Through Week ${throughWeek})</div>
    </div>
    <table class="table">
      <thead>
        <tr>
          ...
          <th>Team</th>
          <th style="width: 100px;">Standings</th>
          <th style="width: 100px;">All Play</th>
          <th style="width: 80px;">ROS</th>
  ...
  currentRankings.forEach(team => {
    ...
    const changeClass = team.change > 0 ? 'rank-up' : team.change < 0 ? 'rank-down' : '';
    const changeText = team.change !== 0 ? Math.abs(team.change) : '';
    
    html += `
      <tr>
        <td>
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
            <span style="font-weight: 700; font-size: 18px;">${team.rank}</span>
            <span class="rank-change ${changeClass}" style="font-size: 14px; min-width: 30px; text-align: right;" title="${team.prevRank ? `Was #${team.prevRank}` : ''}">
              ${changeIcon}${changeText}
            </span>
          </div>
        </td>
        <td>
          <div style="padding: 8px 12px; border-radius: 8px; background-color: ${team.teamColor}33; border-left: 3px solid ${team.teamColor}; font-weight: 600;">
            ${escapeHtml(team.team)}
          </div>
        </td>
        <td><strong>${team.standings}</strong></td>
        <td>${team.allPlay}</td>
        <td>${team.ros}</td>
        <td><span class="schedule-badge schedule-${team.scheduleClass}">${team.schedule}</span></td>
      </tr>
    `;
  });
...
// Helper function to calculate power rankings for a specific week
function calculatePowerRankingsForWeek(rosters, matchupsByWeek, throughWeek, playoffStart, teamSettings) {
  const rankings = [];
  ...
  rosters.forEach(roster => {
    ...
    // Calculate stats through the selected week (regular season only)
    let wins = 0;
    let losses = 0;
    let pointsFor = 0;
    let pointsAgainst = 0;
    ...
    matchupsByWeek.forEach((weekMatchups, week) => {
      if (week > throughWeek || week >= playoffStart) return;
      ...
      if (myMatchup.matchup_id) {
        const opponent = weekMatchups.find(m => 
          m.matchup_id === myMatchup.matchup_id && m.roster_id !== roster.roster_id
        );
        ...
        if (opponent) {
          ...
          // Track opponent's overall record for ROS calculation
          const oppRoster = rosters.find(r => r.roster_id === opponent.roster_id);
          if (oppRoster) {
            // Calculate opponent's record through this week
            let oppW = 0, oppL = 0;
            matchupsByWeek.forEach((wm, w) => {
              if (w > throughWeek || w >= playoffStart) return;
              const om = wm.find(m => m.roster_id === opponent.roster_id);
              if (om && om.matchup_id) {
                const opp2 = wm.find(m => m.matchup_id === om.matchup_id && m.roster_id !== opponent.roster_id);
                if (opp2) {
                  if ((om.points || 0) > (opp2.points || 0)) oppW++;
                  else if ((om.points || 0) < (opp2.points || 0)) oppL++;
                }
              }
            });
            opponentWins += oppW;
      ...
      weekMatchups.forEach(opp => {
        if (opp.roster_id !== roster.roster_id) {
          if ((myMatchup.points || 0) > (opp.points || 0)) {
            allPlayWins++;
          } else if ((myMatchup.points || 0) < (opp.points || 0)) {
            allPlayLosses++;
          }
        }
      });
    ...
    const allPlayTotal = allPlayWins + allPlayLosses;
    const allPlayPct = allPlayTotal > 0 ? allPlayWins / allPlayTotal : 0;
    
    // ROS (Rank of Schedule) - opponent win percentage
    const opponentGames = opponentWins + opponentLosses;
    const ros = opponentGames > 0 ? opponentWins / opponentGames : 0.5;
    
    // Determine schedule difficulty
    let scheduleText, scheduleClass;
    if (ros < 0.45) {
      scheduleText = 'Easy';
      scheduleClass = 'easy';
    } else if (ros > 0.55) {
      ...
      scheduleText = 'Average';
      scheduleClass = 'average';
    }
    
    // Power score calculation (weighted combination)
    const avgPointsFor = throughWeek > 0 ? pointsFor / throughWeek : 0;
    const powerScore = (winPct * 0.35) + (allPlayPct * 0.40) + (avgPointsFor / 150 * 0.25);
    ...
    rankings.push({
      ...
      allPlay: `${allPlayWins}-${allPlayLosses}`,
      ros: (ros * 100).toFixed(0) + '%',
      schedule: scheduleText,
      scheduleClass,
      pointsFor: pointsFor.toFixed(1),
      powerScore
    });
  });
...
function renderAdminTeamList() {
  ...
  // Collect all unique user_ids and their info
  const allUsers = new Map(); // user_id -> { displayName, seasons }
  ...
  histData.seasons.forEach(seasonInfo => {
    ...
    if (rosters) {
      rosters.forEach(roster => {
        if (roster.owner_id) {
          if (!allUsers.has(roster.owner_id)) {
            const user = currentUsers.find(u => u.user_id === roster.owner_id);
            allUsers.set(roster.owner_id, {
              displayName: user ? (user.display_name || user.username || roster.owner_id) : roster.owner_id,
              seasons: []
            });
          }
          allUsers.get(roster.owner_id).seasons.push(season);
        }
      });
    }
  });
  ...
  Array.from(allUsers.entries())
    ...
    .forEach(([userId, info]) => {
      ...
      const customColor = settings.customColor || '#f5c451';
      
      html += `
        <div class="team-settings-row">
          <div>
            <div style="color: var(--text-primary); font-size: 14px;">${escapeHtml(info.displayName)}</div>
            <div style="color: var(--text-muted); font-size: 11px; margin-top: 2px;">${info.seasons.length} season(s)</div>
          </div>
          <input 
            type="text" 
          ...
          <input 
            ...
            data-field="customColor"
          />
          <button class="team-settings-save" data-user-id="${userId}">Save</button>
        </div>
      `;
    });
  
  if (html === '<div class="team-settings-row-header"><div>Original Name</div><div>Custom Name</div><div>Color</div><div>Actions</div></div>') {
    html = '<div class="muted-text">No teams found in league history.</div>';
  }
...
function saveUserSettings(userId) {
  const nameInput = document.querySelector(`.team-settings-input[data-user-id="${userId}"]`);
  const colorInput = document.querySelector(`.team-color-picker[data-user-id="${userId}"]`);
  ...
  // Visual feedback
  const saveBtn = document.querySelector(`.team-settings-save[data-user-id="${userId}"]`);
  if (saveBtn) {
