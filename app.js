  ...
  seasonsToProcess.forEach(seasonInfo => {
    ...
    if (seasonInfo.settings && seasonInfo.settings.playoff_week_start && matchupsByWeek) {
      ...
      if (playoffWeeks.length > 0) {
        ...
        // Try matchup_id 1 first (most common), but also check for the highest scoring matchup
        let champMatchups = championshipWeek.matchups.filter(m => m.matchup_id === 1);
        
        // If no matchup_id 1, find the matchup with highest combined score (likely the championship)
        if (champMatchups.length !== 2) {
          ...
          // Find matchup with highest combined score
          let highestScore = 0;
          let bestMatchup = null;
          matchupsByIdMap.forEach((teams, matchupId) => {
            if (teams.length === 2) {
        ...
        // 3rd place game (usually matchup_id 2 in championship week, or second highest scoring matchup)
        let thirdPlaceMatchups = championshipWeek.matchups.filter(m => m.matchup_id === 2);
        
        if (thirdPlaceMatchups.length !== 2) {
          const matchupsByIdMap = new Map();
          championshipWeek.matchups.forEach(m => {
            if (!m.matchup_id || m.matchup_id === 1) return; // Skip championship matchup
            if (!matchupsByIdMap.has(m.matchup_id)) {
              matchupsByIdMap.set(m.matchup_id, []);
            }
            matchupsByIdMap.get(m.matchup_id).push(m);
          ...
          // Find matchup with second highest combined score
          let secondHighest = 0;
          let bestMatchup = null;
          matchupsByIdMap.forEach((teams, matchupId) => {
            if (teams.length === 2) {
          ...
          if (bestMatchup) {
            thirdPlaceMatchups = bestMatchup;
          }
        }
        ...
        // Count playoff wins - ONLY for teams in the main playoff bracket
        // Determine which teams made playoffs based on regular season standings
        const regularSeasonStandings = new Map(); // roster_id -> wins
          ...
          if (week < playoffStart) { // Only regular season
            weekMatchups.forEach(matchup => {
              ...
              if (!regularSeasonStandings.has(matchup.roster_id)) {
                regularSeasonStandings.set(matchup.roster_id, 0);
              }
              ...
              if (opponent && (matchup.points || 0) > (opponent.points || 0)) {
                regularSeasonStandings.set(matchup.roster_id, regularSeasonStandings.get(matchup.roster_id) + 1);
              }
            });
          }
        });
        
        // Get top X teams (playoff teams)
        const playoffRosterIds = new Set(
          Array.from(regularSeasonStandings.entries())
            .sort((a, b) => b[1] - a[1])
        ...
        playoffWeeks.forEach(({ week, matchups }) => {
          matchups.forEach(matchup => {
            ...
            // Only count wins for teams that made the main playoffs
